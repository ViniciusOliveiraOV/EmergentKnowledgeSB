...
Why the architecture is the way it is: `_system/adr/`. Read those before
...
Topic (climate, AI, philosophy) is expressed by tags and links, never by folder.
    ...
    _system/      architecture docs, validator, fixtures  (meta)
...
[[ONTOLOGY]] § Canonical language. Two clocks — `created` (system time) vs.
`asserted_at` (knowledge time) — are settled in [[ONTOLOGY]] § Temporal
...
relations:
  - rel: contradicts
    target: "[[Escalar Antes de Validar]]"
    epistemic: user_position
    source: src-20260826-chatgpt-fixture-01
...
### Vocabulary (closed set — extending it is a human edit to [[ONTOLOGY]])
...
with zero plugins. See [[ONTOLOGY]] § Epistemic status for the six values.
...
framework architecture.** It belongs in [[INSTANCE]], never here — recording
...
[[AGENT_RULES]]. No client gets privileged state. If a client's memory
