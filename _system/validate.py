#!/usr/bin/env python3
"""Frontmatter/schema validator for the vault. Run from the vault root.

    python3 _system/validate.py [--warnings-are-errors]

Exit 0 = clean. Read-only: this script never writes to the vault.
"""
...
# repo-convention files at the vault root, not notes
# repo-convention files at the root: not vault notes. README.md must stay
# frontmatter-free so GitHub renders it correctly.
...
# is honest, so YYYY / YYYY-MM / YYYY-MM-DD all pass (ONTOLOGY § Temporal).
...
def check(path):
    ...
    if not m:
        return err("no YAML frontmatter")
    try:
        fm = yaml.safe_load(m.group(1)) or {}
    except yaml.YAMLError as e:
        return err(f"unparseable frontmatter: {e}")
    if not isinstance(fm, dict):
        return err("frontmatter is not a mapping")
    body = m.group(2)
    ...
    top = rel.parts[0] if len(rel.parts) > 1 else ""   # "" = vault root
    if top not in folders:
        # fixtures deliberately live outside their type's folder
        (warn if "fixtures" in rel.parts else err)(
            f"type {t} lives in {sorted(folders)}, found in {top}/")
    ...
    elif tr is None and t in ("doc",):
        warn("no track: declared — is this core or instance?")
    ...
    if t == "source":
        ...
        if not h:
            warn("source has no content_hash (L0 integrity unchecked)")
        elif h != actual:
            err(f"content_hash mismatch — L0 body was edited. "
...
def main():
    ...
    for e in errors:
        print(f"ERROR {e}")
    strict = "--warnings-are-errors" in sys.argv
    print(f"\n{len(files)} notes · {len(errors)} errors · {len(warnings)} warnings")
...
def selftest():
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        good.write_text(good.read_text().replace("asserted_at: 2026-03",
                                                 "asserted_at: 2027"))
        errors, warnings = [], []
        check(good)
